...
constexpr static int NUM_ELEMENTS = 4000000;
...
int main(int argc, char* argv[]) {
    ...
    if (program.is_subcommand_used(simulate_command)) {
        if (simulate_command.is_used("quantity")) {
            cout << "Simulating quantity: "
                 << simulate_command.get<int>("quantity") << endl;
        }
    }
    ...
    vector<Eigen::Matrix2f> res(NUM_ELEMENTS);
...
#pragma omp parallel for
    ...
    auto elapsed = chrono::duration_cast<chrono::nanoseconds>(
